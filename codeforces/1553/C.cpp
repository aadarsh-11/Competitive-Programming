...
int rec(string s, int a, int b, int i)
{
    if (i == s.size())
    {
        return s.size();
    ...
    if (i % 2)
    {
        ...
        {
            return min(rec(s, a, b + 1, i + 1), rec(s, a, b, i + 1));
        }
    }
    else
    {
        ...
        {
            return min(rec(s, a + 1, b, i + 1), rec(s, a, b, i + 1));
        }
    }
    // return 0;
}
...
void solve()
{
    ll ans = 10, cnt = 0, sum = 0, temp = 0, t1 = 0, t2 = 0;
    ...
    // }
    // if (a[0] == '?')
    //     ans = min(rec(s, a, b, 0, 0), rec(s, a, b, 0, 1));
    // else
    // {
    //     ans = rec(s, a, b, 0, s[0] - '0');
    // }
    // cout<< rec(s,a,b,0)<<endl;
    for (int i = 0; i < 10; ++i)
    {
        if (i % 2 == 0)
        {
