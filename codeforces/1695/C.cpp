...
void solve()
{
	...
	vector<vector<pair<int, int>>> b(n, vpii(m));

	int x = 0;
	fo(i, m)
	{
		b[0][i] = {x + a[0][i], x + a[0][i]};
		x = b[0][i].f;
	}
	x = 0;
	fo(i, n)
	{
		b[i][0] = {x + a[i][0], x + a[i][0]};
		x = b[i][0].f;
	}
	...
	for (int i = 1; i < n ; i++)
	{
		for (int j = 1 ; j < m ; j ++)
		{
			b[i][j] = {min(b[i - 1][j].f, b[i][j - 1].f) + a[i][j], max(b[i - 1][j].se, b[i][j - 1].se) + a[i][j]};
	...
	string res = "NO";
	if (l <= 0 and r >= 0 and (n + m) % 2)
		res = "YES";
	// cout << b[n - 1][m - 1].f << b[n - 1][m - 1].se << endl;
	cout << res << endl;
}
