...
int rec(string s, int i, vi dps, int flag = 0)
{
    ...
    if (flag)
    {
        ...
        return (dps[s[i] - '0'+10] * rec(s, i + 2, dps) + dps[s[i] - '0'+9] * rec(s, i + 2, dps, 1));
    ...
    if(s[i] == '0')
    {
        return dps[s[i] - '0'] * rec(s, i + 2, dps);
    }
    return (dps[s[i] - '0'] * rec(s, i + 2, dps) + dps[s[i] - '0' - 1] * rec(s, i + 2, dps, 1));
}
...
void solve()
{
    ...
    string s = n;
    // deb2(rec(s, 0, dps),rec(s, 1, dps));
    ans = rec(s, 0, dps) * rec(s,1,dps);
    cout<<ans-2<<endl;
}
